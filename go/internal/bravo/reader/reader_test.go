...
func TestReaderValidMinimal(t *testing.T) {
	...
	if events[0].Type != diagnostic.EventVersion {
		t.Errorf("event 0: expected Version, got %v", events[0].Type)
	}
	if events[1].Type != diagnostic.EventPlan {
		t.Errorf("event 1: expected Plan, got %v", events[1].Type)
	}
	if events[2].Type != diagnostic.EventTestPoint {
...
func TestReaderANSIColoredStream(t *testing.T) {
	...
	var tpEvents []diagnostic.Event
	for _, ev := range events {
		if ev.Type == diagnostic.EventTestPoint {
	...
	if tpEvents[2].TestPoint.Directive != diagnostic.DirectiveSkip {
...
func TestReaderStreamedOutputPragma(t *testing.T) {
	...
	var commentEvents []diagnostic.Event
	for _, ev := range events {
		if ev.Type == diagnostic.EventComment {
...
func TestReaderStreamedOutputSubtestIsolation(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == diagnostic.EventComment && ev.Comment == "parent comment" {
			if !ev.StreamedOutput {
				t.Error("expected parent comment to have StreamedOutput=true")
			}
		}
		if ev.Type == diagnostic.EventComment && ev.Comment == "child comment" {
			if ev.StreamedOutput {
...
func TestReaderStreamedOutputNotActiveByDefault(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == diagnostic.EventComment && ev.StreamedOutput {
...
func TestReaderLocaleFormattedTestPoint(t *testing.T) {
	...
	events, _, _ := collectEvents(input)
	for _, ev := range events {
		if ev.Type == diagnostic.EventTestPoint && ev.TestPoint.Number == 1234 {
			return
		}
	}
...
func TestReaderLocaleRoundTrip(t *testing.T) {
	...
	tw := writer.NewLocaleWriter(&buf, language.MustParse("en-US"))
	for i := 0; i < 1234; i++ {
...
func TestReaderOutputBlock(t *testing.T) {
	...
	ev, err := r.Next()
	if err != nil || ev.Type != diagnostic.EventVersion {
		t.Fatalf("expected version, got %v %v", ev, err)
	}

	ev, err = r.Next()
	if err != nil || ev.Type != diagnostic.EventOutputHeader {
	...
	ev, err = r.Next()
	if err != nil || ev.Type != diagnostic.EventOutputLine {
	...
	ev, err = r.Next()
	if err != nil || ev.Type != diagnostic.EventTestPoint {
		t.Fatalf("expected test point, got %v %v", ev, err)
	}

	ev, err = r.Next()
	if err != nil || ev.Type != diagnostic.EventPlan {
