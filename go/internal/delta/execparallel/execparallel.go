...
func ConvertExecParallel(results <-chan ExecResult, w io.Writer, verbose bool, color bool) int {
	tw := writer.NewColorWriter(w, color)
	exitCode := 0
...
func emitExecResult(tw *writer.Writer, r ExecResult, verbose bool) bool {
...
func emitInnerTAPSubtest(tw *writer.Writer, r ExecResult) bool {
	child := tw.Subtest("%s", r.Command)
	summary, _ := replay.Replay(bytes.NewReader(r.Stdout), child)
...
func startStatusTicker(tw *writer.Writer, spinner *statusSpinner, mu *sync.Mutex, content *string) func() {
...
func runWithStatusLine(ctx context.Context, tw *writer.Writer, spinner *statusSpinner, arg, command string, verbose bool) bool {
	var mu sync.Mutex
...
func parallelStatusLine(running, done, total int, color bool) string {
	if color {
		return fmt.Sprintf("%s [%d/%d done]",
			style.Yellow.Render(fmt.Sprintf("%d running", running)),
			done, total)
	}
...
func execResultDiagnostics(r ExecResult) *yaml_diagnostic.YAMLDiagnostic {
	d := &yaml_diagnostic.YAMLDiagnostic{
