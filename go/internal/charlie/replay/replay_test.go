...
func replayInto(t *testing.T, input string) (string, diagnostic.Summary, *writer.Writer) {
	t.Helper()
	var buf bytes.Buffer
	tw := writer.NewWriter(&buf)
	header := buf.Len()
...
func TestReplayInsideSubtestProducesFirstClassChildTAP(t *testing.T) {
	...
	r := reader.NewReader(strings.NewReader(out))
	s := r.Summary()
	if s.Failed == 0 {
