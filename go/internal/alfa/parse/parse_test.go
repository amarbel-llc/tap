...
func TestParseTestPoint(t *testing.T) {
	tests := []struct {
		...
		{"ok", true, 0, "", diagnostic.DirectiveNone, ""},
		{"ok 1", true, 1, "", diagnostic.DirectiveNone, ""},
		{"ok 1 - first test", true, 1, "first test", diagnostic.DirectiveNone, ""},
		{"not ok 2 - failing", false, 2, "failing", diagnostic.DirectiveNone, ""},
		{"ok 3 - skipped # SKIP not applicable", true, 3, "skipped", diagnostic.DirectiveSkip, "not applicable"},
		{"not ok 4 - todo # TODO not done", false, 4, "todo", diagnostic.DirectiveTodo, "not done"},
		{"ok - no number", true, 0, "no number", diagnostic.DirectiveNone, ""},
		{"not ok - also no number", false, 0, "also no number", diagnostic.DirectiveNone, ""},
		{"ok 1 - has \\# escaped hash", true, 1, "has # escaped hash", diagnostic.DirectiveNone, ""},
	}
	for _, tt := range tests {
...
func TestDirectiveCase(t *testing.T) {
	tests := []struct {
		...
		{"ok 1 - x # SKIP reason", diagnostic.DirectiveSkip},
		{"ok 1 - x # skip reason", diagnostic.DirectiveSkip},
		{"ok 1 - x # Skip reason", diagnostic.DirectiveSkip},
		{"ok 1 - x # TODO reason", diagnostic.DirectiveTodo},
		{"ok 1 - x # todo reason", diagnostic.DirectiveTodo},
		{"ok 1 - x # Todo reason", diagnostic.DirectiveTodo},
	}
	for _, tt := range tests {
