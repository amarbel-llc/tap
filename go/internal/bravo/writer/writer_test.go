...
func TestWriteDiagnosticsNamedFields(t *testing.T) {
	var buf bytes.Buffer
	yaml_diagnostic.WriteDiagnostics(&buf, &yaml_diagnostic.YAMLDiagnostic{
		Message:  "something broke",
		Severity: "fail",
		File:     "main.go",
		Line:     42,
...
func TestWriteDiagnosticsNil(t *testing.T) {
	var buf bytes.Buffer
	yaml_diagnostic.WriteDiagnostics(&buf, nil, false)
	if buf.Len() != 0 {
...
func TestWriteAllNotOkWithDiagnostics(t *testing.T) {
	...
	tw := NewWriter(&buf)
	tw.WriteAll(slices.Values([]TestPoint{
		{Description: "failing", Ok: false, Diagnostics: &yaml_diagnostic.YAMLDiagnostic{
			Message:  "broke",
			Severity: "fail",
		}},
	}))
	...
	tw.WriteAll(slices.Values([]TestPoint{
		{Description: "outer", Subtests: func(sub *Writer) {
			sub.WriteAll(slices.Values([]TestPoint{
				...
				{Description: "inner-b", Ok: false, Diagnostics: &yaml_diagnostic.YAMLDiagnostic{
					Message: "broke",
				}},
			}))
		}},
...
func TestWriteAllOutputValidatesWithReader(t *testing.T) {
	...
	tw.WriteAll(slices.Values([]TestPoint{
		...
		{Description: "fail", Ok: false, Diagnostics: &yaml_diagnostic.YAMLDiagnostic{
			Message: "broke",
		}},
		{Description: "skipped", Skip: "not ready"},
	...
	r := reader.NewReader(strings.NewReader(buf.String()))
	summary := r.Summary()
	if !summary.Valid {
		diags := r.Diagnostics()
		for _, d := range diags {
...
func TestWriteAllOkWithDiagnostics(t *testing.T) {
	...
	tw := NewWriter(&buf)
	tw.WriteAll(slices.Values([]TestPoint{
		{Description: "pass with info", Ok: true, Diagnostics: &yaml_diagnostic.YAMLDiagnostic{
...
func TestOutputBlock(t *testing.T) {
	var buf bytes.Buffer
	tw := NewWriter(&buf)
	tw.OutputBlock("build the project", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
...
func TestOutputBlockNotOk(t *testing.T) {
	...
	tw.OutputBlock("build the project", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		ob.Line("compiling main.rs")
		return &yaml_diagnostic.YAMLDiagnostic{Message: "compilation failed", Severity: "fail"}
	})
	tw.Plan()
	got := buf.String()
...
func TestOutputBlockColorStripsNonSGR(t *testing.T) {
	...
	tw.OutputBlock("test", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
...
func TestOutputBlockEmpty(t *testing.T) {
	var buf bytes.Buffer
	tw := NewWriter(&buf)
	tw.OutputBlock("no output", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		return nil
	})
	tw.Plan()
	want := "TAP version 14\n" +
...
func TestOutputBlockLazyHeaderFiresOnFirstLine(t *testing.T) {
	var buf bytes.Buffer
	tw := NewWriter(&buf)
	tw.OutputBlock("build", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		ob.Line("step 1")
		ob.Line("step 2")
		return nil
	})
...
func TestOutputBlockEmptyValidatesWithReader(t *testing.T) {
	...
	tw.OutputBlock("with output", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		ob.Line("hello")
		return nil
	})
	tw.Plan()
...
func TestLazyOutputHeaderAcrossAllMethodsValidates(t *testing.T) {
	...
	tw := NewWriter(&buf)
	tw.Ok("a")
	tw.OkDiag("b", &yaml_diagnostic.YAMLDiagnostic{Message: "note"})
	tw.NotOk("c", map[string]string{"message": "boom"})
	tw.Skip("d", "later")
	tw.SkipDiag("e", "soon", &yaml_diagnostic.YAMLDiagnostic{Message: "why"})
	tw.Todo("f", "pending")
	tw.OutputBlock("g", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic { return nil })
	tw.OutputBlock("h", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		ob.Line("line")
		return nil
	})
	tw.Plan()
...
func TestWriteAllOutputBlock(t *testing.T) {
	...
	tw.WriteAll(func(yield func(TestPoint) bool) {
		yield(TestPoint{
			Description: "compile",
			Ok:          true,
			OutputBlock: func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
				ob.Line("building...")
				return nil
			},
		})
...
func TestOutputBlockRoundTrip(t *testing.T) {
	...
	tw.OutputBlock("test", func(ob *OutputBlockWriter) *yaml_diagnostic.YAMLDiagnostic {
		...
		return &yaml_diagnostic.YAMLDiagnostic{Message: "1 test failed", Severity: "fail"}
	})
	tw.Plan()
